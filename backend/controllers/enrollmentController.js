...
exports.getAllEnrollments = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Error al obtener la lista de matrículas." });
    }
};
...
exports.getEnrollmentById = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Error al obtener detalles de la matrícula." });
    }
};
...
exports.calculateEnrollmentCosts = async (req, res) => {
    ...
    if (!periodo_id) {
        return res.status(400).json({ message: "El ID del período es obligatorio para calcular costos." });
    }
...
exports.createEnrollment = async (req, res) => {
    ...
    if (!estudiante_id || !apoderado_id ||!seccion_id || !periodo_id) {
        return res.status(400).json({ message: "Estudiante, período y sección son campos obligatorios." });
    }

    try {
        const studentExists = await Student.findById(estudiante_id);
        if (!studentExists) return res.status(404).json({ message: "Estudiante no encontrado." });
        ...
        const newEnrollment = await Enrollment.create({ estudiante_id, apoderado_id: apoderado_id || null, seccion_id, periodo_id });

        res.status(201).json({ message: "Matrícula creada exitosamente.", enrollment: newEnrollment });
...
exports.updateEnrollment = async (req, res) => {
    ...
    const { estudiante_id, apoderado_id, seccion_id, periodo_id, estado } = req.body;

    if (!estudiante_id || !seccion_id || !periodo_id || !estado) {
        return res.status(400).json({ message: "Estudiante, período, sección y estado son obligatorios." });
    }

    try {
        const success = await Enrollment.update(enrollmentId, { estudiante_id, apoderado_id: apoderado_id || null, seccion_id, periodo_id, estado });

        if (!success) {
            return res.status(404).json({ message: "Matrícula no encontrada para actualizar." });
        }
