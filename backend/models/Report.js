...
const Report = {
  ...
  getStudentFullReport: async (studentId) => {
    // Obtiene todas las matrículas, cuotas y pagos para un estudiante
    const [rows] = await pool.execute(
      `
            SELECT 
                m.id AS matricula_id, m.estado AS matricula_estado, m.created_at AS fecha_matricula,
                p.nombre AS periodo_nombre, 
                g.nombre AS grado, s.nombre AS seccion,
                c.id AS cuota_id, c.concepto AS cuota_concepto, c.monto AS cuota_monto_obligatorio, c.fecha_limite AS cuota_fecha_limite,
                pa.id AS pago_id, pa.monto AS pago_monto, pa.fecha_pago, pa.referencia_pago, mt.nombre AS metodo_pago_nombre
            FROM matriculas m
            ...
            WHERE e.id = ?
            ORDER BY m.created_at DESC, c.orden, pa.fecha_pago
        `,
      [studentId]
    );
    return rows;
  ...
  // 3. Histórico de Pagos (Para filtros avanzados)
  getPaymentHistory: async (startDate, endDate, methodId) => {
    let query = `
            SELECT 
                pa.id, pa.monto, pa.fecha_pago, pa.referencia_pago,
                e.primer_nombre AS est_nombre, e.primer_apellido AS est_apellido,
                mt.nombre AS metodo, c.concepto AS cuota_concepto
            FROM pagos pa
    ...
    if (startDate && endDate) {
      query += " AND pa.fecha_pago BETWEEN ? AND ?";
